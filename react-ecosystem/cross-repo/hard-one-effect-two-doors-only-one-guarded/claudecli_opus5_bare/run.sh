#!/bin/zsh
# claudecli_opus5_bare — hard-one-effect-two-doors-only-one-guarded
# Opus 5 with filesystem tools over the pinned checkouts and nothing else.
...
set -e
ARM=/Users/sauravverma/programs/benchmarks/react-ecosystem/cross-repo/hard-one-effect-two-doors-only-one-guarded/claudecli_opus5_bare
CWD=/tmp/xrepo-runs/hard-one-effect-two-doors-only-one-guarded-bare
...
# (this surface needs no local index)
...
echo "arm finished: hard-one-effect-two-doors-only-one-guarded bare $(cat $ARM/wall.txt)"
echo
echo "next: python3 /Users/sauravverma/programs/benchmarks/react-ecosystem/score_arm.py cross-repo/hard-one-effect-two-doors-only-one-guarded claudecli_opus5_bare"
