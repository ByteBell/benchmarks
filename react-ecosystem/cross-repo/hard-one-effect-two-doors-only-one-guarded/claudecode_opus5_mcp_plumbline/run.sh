#!/bin/zsh
# claudecode_opus5_mcp_plumbline — hard-one-effect-two-doors-only-one-guarded
...
set -e
ARM=/Users/sauravverma/programs/benchmarks/react-ecosystem/cross-repo/hard-one-effect-two-doors-only-one-guarded/claudecode_opus5_mcp_plumbline
CWD=/tmp/xrepo-runs/hard-one-effect-two-doors-only-one-guarded-plumbline
...
[ -f "/Users/sauravverma/programs/benchmarks/react-ecosystem/mcp_plumbline.json" ] || { echo "MISSING: /Users/sauravverma/programs/benchmarks/react-ecosystem/mcp_plumbline.json"; exit 1; }
...
[ -f /Users/sauravverma/programs/benchmarks/react-ecosystem/cross-repo/hard-one-effect-two-doors-only-one-guarded/golden.json ] || { echo "NO golden.json at /Users/sauravverma/programs/benchmarks/react-ecosystem/cross-repo/hard-one-effect-two-doors-only-one-guarded/golden.json - gate 4 cannot prove anything"; exit 1; }
sandbox-exec -f $PROFILE /bin/cat /Users/sauravverma/programs/benchmarks/react-ecosystem/cross-repo/hard-one-effect-two-doors-only-one-guarded/golden.json >/dev/null 2>&1 \
...
( cd $CWD && sandbox-exec -f $PROFILE "$CLAUDE" -p "$(cat $ARM/run_prompt.txt)" \
    ...
    --mcp-config "/Users/sauravverma/programs/benchmarks/react-ecosystem/mcp_plumbline.json" --strict-mcp-config \
    --allowedTools "mcp__plumbline__roll_call" "mcp__plumbline__blueprint" "mcp__plumbline__stakeout" "mcp__plumbline__manhunt" "mcp__plumbline__dragnet" "mcp__plumbline__cross_repo_lookup" "mcp__plumbline__case_file" "mcp__plumbline__case_notes" "mcp__plumbline__cold_case" "mcp__plumbline__collateral_damage" "mcp__plumbline__evidence_locker" "mcp__plumbline__file_a_complaint" "mcp__plumbline__interrogation" "mcp__plumbline__kingpin" "mcp__plumbline__mugshot" "mcp__plumbline__paper_trail" "mcp__plumbline__pull_the_evidence" "mcp__plumbline__read_the_fine_print" "mcp__plumbline__shakedown" "mcp__plumbline__the_receipts" \
...
echo "arm finished: hard-one-effect-two-doors-only-one-guarded plumbline $(cat $ARM/wall.txt)"
echo
echo "next: python3 /Users/sauravverma/programs/benchmarks/react-ecosystem/score_arm.py cross-repo/hard-one-effect-two-doors-only-one-guarded claudecode_opus5_mcp_plumbline"
